...
# ============================================================
with tab1:
    st.caption("Monthly revenue trends, order volumes and customer spend patterns across the Olist platform.")
    ...
    if len(date_range) == 2:
        df = df[
            (df["order_month"] >= pd.Timestamp(date_range[0])) &
    ...
    st.caption("Revenue peaked in late 2017 and mid 2018 driven by seasonal demand spikes.")
    ...
    st.caption("Order volume tracks closely with revenue — growth was consistent through 2017 into 2018.")
    ...
    st.caption("AOV fluctuates month to month — spikes may indicate high-value product categories gaining traction.")
    ...
    st.caption("Revenue per customer helps identify whether growth is driven by new acquisition or increased spend.")
    ...
    st.caption("New customer acquisition was the primary growth driver — repeat purchase rate on Olist is low.")
    ...
    st.caption("Strong positive correlation between order volume and revenue — no significant outlier months.")

    st.subheader("Monthly Revenue Data")
    st.dataframe(df, use_container_width=True)
    st.caption("Full monthly breakdown. Use the date filter in the sidebar to zoom into specific periods.")
...
# ============================================================
with tab2:
    st.caption("Late delivery rates, actual vs estimated delivery times and logistics performance by Brazilian state.")
    ...
    col3.metric("Overall Late Rate",
                f"{df['late_orders'].sum() / df['total_orders'].sum() * 100:,.1f}%"
                if df['total_orders'].sum() > 0 else "N/A")
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption("States in the north and northeast tend to have higher late delivery rates due to distance from seller hubs.")
    ...
    st.plotly_chart(fig2, use_container_width=True)
    st.caption("São Paulo and Rio de Janeiro dominate order volume — expected given population density.")
    ...
    st.plotly_chart(fig3, use_container_width=True)
    st.caption("Estimated delivery dates are generally conservative — actual delivery often exceeds estimates in remote states.")
    ...
    st.plotly_chart(fig4, use_container_width=True)
    st.caption("Positive gap means delivered later than estimated. Negative means delivered early — a good signal.")

    seller_state_df = df.groupby("seller_state").agg(
        total_orders=("total_orders", "sum"),
        late_orders=("late_orders", "sum"),
    ).reset_index()
    seller_state_df["late_rate_pct"] = (
        seller_state_df["late_orders"] / seller_state_df["total_orders"] * 100
    ).round(2)
    ...
    st.plotly_chart(fig5, use_container_width=True)
    st.caption("Most sellers are concentrated in São Paulo — seller state has a strong influence on delivery speed.")
    ...
    st.plotly_chart(fig6, use_container_width=True)
    st.caption("São Paulo accounts for the majority of seller activity on the platform.")
    ...
    st.caption("Darker cells indicate high late delivery rates between that customer-seller state combination.")

    st.subheader("Raw Delivery Data")
    st.dataframe(df, use_container_width=True)
    st.caption("Raw delivery data filtered by your sidebar selections.")
...
# ============================================================
with tab3:
    st.caption("Customer lifetime value segmentation, spend distribution and review behaviour across regions.")
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption("The vast majority of customers fall into the low-value tier — typical for marketplace platforms with low repeat rates.")
    ...
    fig2 = px.histogram(df, x="total_spent", nbins=80,
                        ...
                        color_discrete_sequence=["#636EFA"])
    st.plotly_chart(fig2, use_container_width=True)
    st.caption("Spend is heavily right-skewed — most customers make a single small purchase.")
    ...
    fig3 = px.histogram(df, x="total_orders", nbins=20,
                        ...
                        color_discrete_sequence=["#EF553B"])
    st.plotly_chart(fig3, use_container_width=True)
    st.caption("Most customers place only one order, confirming low repeat purchase behaviour on the platform.")
    ...
    st.plotly_chart(fig4, use_container_width=True)
    st.caption("Southern states tend to show higher average spend per customer.")
    ...
    st.plotly_chart(fig5, use_container_width=True)
    st.caption("São Paulo has by far the largest customer base, reflecting Brazil's population distribution.")
    ...
    st.plotly_chart(fig6, use_container_width=True)
    st.caption("Higher spend does not strongly correlate with higher review scores — satisfaction is driven by other factors.")
    ...
    st.plotly_chart(fig7, use_container_width=True)
    st.caption("High-value customers are typically those with multiple orders rather than single large purchases.")
    ...
    st.caption("Most customers have a lifespan of zero days — meaning they only ordered once. A key retention insight.")
    ...
    st.caption("Top 20 customers by total spend. High-value customers worth targeting for retention campaigns.")
...
# ============================================================
with tab4:
    st.caption("Seller revenue tiers, delivery reliability and regional performance across the Olist seller network.")
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Most sellers fall into the low tier — revenue on the platform is highly concentrated among a few top sellers.")
    ...
    fig2 = px.histogram(df, x="total_revenue", nbins=60,
                        ...
                        color_discrete_sequence=["#636EFA"])
    st.plotly_chart(fig2, use_container_width=True)
    st.caption("Revenue distribution is heavily skewed — a small number of sellers generate the majority of platform revenue.")
    ...
    fig3 = px.histogram(df, x="late_delivery_rate_pct", nbins=40,
                        ...
                        color_discrete_sequence=["#EF553B"])
    st.plotly_chart(fig3, use_container_width=True)
    st.caption("Most sellers maintain a late delivery rate below 20% — outliers worth investigating for operational issues.")
    ...
    st.plotly_chart(fig4, use_container_width=True)
    st.caption("No strong relationship between revenue and late delivery rate — high earners can still be unreliable.")
    ...
    st.plotly_chart(fig5, use_container_width=True)
    st.caption("São Paulo dominates seller revenue — reflects both seller concentration and proximity to customer base.")
    ...
    st.plotly_chart(fig6, use_container_width=True)
    st.caption("Seller supply is heavily concentrated in the southeast — a potential logistics bottleneck for other regions.")
    ...
    st.plotly_chart(fig7, use_container_width=True)
    st.caption("States with fewer sellers tend to have higher late delivery rates due to longer shipping distances.")
    ...
    st.plotly_chart(fig8, use_container_width=True)
    st.caption("Revenue scales with items sold but with variance — average item price plays a significant role.")
    ...
    st.caption("Higher priced items tend to carry higher freight costs — likely reflecting size and weight.")
    ...
    st.dataframe(top_df, use_container_width=True)
    st.caption("Top 20 sellers by revenue with delivery reliability scores. Strong sellers balance both metrics.")